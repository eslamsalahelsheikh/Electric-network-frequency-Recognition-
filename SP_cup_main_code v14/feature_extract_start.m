...
%     a=abs(fft(s,120));
    ...
    b=abs(dct(s,120));
    b=b(1:6);
    m = [log((moment(s,3))) log(abs(moment(s,4))) log(abs(moment(s,5))) log(abs(moment(s,6))) log(abs(moment(s,7))) log(abs(skewness(s))) kurtosis(s)];
...
noz=find(dd==0);
Features = [Features log(range(dd)) log(abs(moment(dd,4))) length(noz)];
